...
| One-page invoice, rendered concurrently on 16 cores | 26 KB | — | ~1,400 docs/sec (~84,000/min) |
...
A template is loaded once (`OfficeTemplate.fromBytes`) and can then be rendered any number of times with different data. Rendering is CPU-bound and stateless, so throughput scales linearly with cores — and horizontally with application instances, since there is no shared state or external service.
